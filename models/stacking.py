...
class DeepStack:
    ...
    def init_models_layer1(self):
        ...
        LGB_L1 = regressors.LightGBM(self.x_g_train, self.y_train, self.w_train, self.e_train,
                                     self.x_g_test, self.id_test, self.num_boost_round_lgb_l1)
        XGB_L1 = regressors.XGBoost(self.x_train, self.y_train, self.w_train, self.e_train,
    ...
    def init_models_layer2(self):
        ...
        LGB_L2 = regressors.LightGBM(self.x_g_train, self.y_train, self.w_train, self.e_train,
    ...
    def train_models(self, models_blender, params, x_train, y_train, w_train, x_g_train,
        ...
        for iter_model, model in enumerate(models_blender):
            ...
            all_model_valid_prob.append(pred_valid)
    ...
    def stack_final_layer(self, model_name, params, n_valid, n_cv, n_era, x_outputs,
        ...
        if model_name == 'LGB':
            ...
            model = regressors.LightGBM(x_g_outputs, self.y_train, self.w_train, self.e_train, test_g_outputs,
...
class StackTree:
    ...
    def layer1_initializer(self, x_train, x_g_train, x_test, x_g_test):
        ...
        LGB_L1 = regressors.LightGBM(x_g_train, self.y_train, self.w_train, self.e_train,
                                     x_g_test, self.id_test, num_boost_round=self.num_boost_round_lgb_l1)
        XGB_L1 = regressors.XGBoost(x_train, self.y_train, self.w_train, self.e_train,
                                    x_test, self.id_test, num_boost_round=self.num_boost_round_xgb_l1)
        AB_L1 = regressors.AdaBoost(x_train, self.y_train, self.w_train,
                                    self.e_train, x_test, self.id_test)
        RF_L1 = regressors.RandomForest(x_train, self.y_train, self.w_train,
                                        self.e_train, x_test, self.id_test)
        ET_L1 = regressors.ExtraTrees(x_train, self.y_train, self.w_train,
                                      self.e_train, x_test, self.id_test)
        GB_L1 = regressors.GradientBoosting(x_train, self.y_train, self.w_train,
                                            self.e_train, x_test, self.id_test)
    ...
    def layer2_initializer(self, x_train, x_g_train, x_test, x_g_test):
        ...
        LGB_L2 = regressors.LightGBM(x_g_train, self.y_train, self.w_train, self.e_train,
    ...
    def final_layer_initializer(self, blender_x_tree, blender_test_tree, blender_x_g_tree,
        ...
        if self.model_final == 'lgb':
            LGB_END = regressors.LightGBM(blender_x_g_tree, self.y_train, self.w_train, self.e_train,
...
class PrejudgeStackLayer:
    ...
    def models_trainer(self, models_blender, params, x_train, y_train, w_train, e_train, x_g_train,
        ...
        for iter_model, model in enumerate(models_blender):
            ...
            pred_valid_p, prob_test_p, losses_p = \
                model.prejudge_stack_train(x_train_p, x_g_train_p, y_train_p, w_train_p, e_train_p, x_valid_p,
                                           ...
                                           save_final_pred=True, save_final_pred_train=False, save_csv_log=True,
                                           csv_idx='prejudge_stack_p', return_prob_test=False)

            print('======================================================')
            print('Training Models of Negative Era Sign...')
            pred_valid_n, prob_test_n, losses_n = \
            ...
            pred_valid = np.zeros_like(self.y_train, dtype=np.float64)
            prob_test = np.zeros_like(self.id_test, dtype=np.float64)

            for idx_p, prob_p in zip(era_idx_valid_p, pred_valid_p):
                pred_valid[idx_p] = prob_p
            for idx_n, prob_n in zip(era_idx_valid_n, pred_valid_n):
                pred_valid[idx_n] = prob_n
